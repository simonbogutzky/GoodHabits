...
public class Item: NSManagedObject {
    public required convenience init(context: NSManagedObjectContext, name: String, timestamp: Date = Date()) {
        ...
        for i in 0..<dayCount {
            ...
            day.isVisible = true
            day.date = midnight.addingTimeInterval(Double(i) * 60.0 * 60.0 * 24.0)
            self.addToDays(day)
        }
        ...
        for i in 0..<leadingDays {
            let day = Day(context: context)
            day.date = previousMondayMidnight.addingTimeInterval(Double(i) * 60.0 * 60.0 * 24.0)
            self.addToDays(day)
        }
        ...
        for i in 0..<trailingDays {
            let day = Day(context: context)
            day.date = lastDayMidnight.addingTimeInterval(Double(i) * 60.0 * 60.0 * 24.0)
            self.addToDays(day)
        }
    }
}
